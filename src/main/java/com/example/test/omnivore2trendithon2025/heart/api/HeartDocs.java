...
public interface HeartDocs {
    ...
    @Operation(summary = "호출했을 때 케이크 좋아요 등록/삭제", description = "좋아요가 안 되어있으면 등록, 좋아요가 되어있으면 삭제 진행.",
            responses = {
                    @ApiResponse(responseCode = "200", description = "케이크 좋아요 등록/삭제 성공"),
    ...
    @Operation(summary = "호출했을 때 컵케이크 좋아요 등록/삭제", description = "좋아요가 안 되어있으면 등록, 좋아요가 되어있으면 삭제 진행.",
            responses = {
                    @ApiResponse(responseCode = "200", description = "컵케이크 좋아요 등록/삭제 성공"),
                    @ApiResponse(responseCode = "400", description = "잘못된 요청"),
                    @ApiResponse(responseCode = "401", description = "인증 실패"),
                    @ApiResponse(responseCode = "500", description = "서버 오류")
            })
    RspTemplate<SaveCupCakeResponse> createOrDeleteCupCakeHeart(
            @Parameter(description = "로그인한 유저의 이메일(토큰에서 자동 추출)", hidden = true) String email,
            @Parameter(description = "cup-cake 고유 Id", required = true) Long cupCakeId);
}